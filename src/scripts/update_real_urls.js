/**
 * Script to update services with real SetScript URLs
 */
...
async function updateRealUrls() {
  try {
    console.log('Updating services with real SetScript URLs...');
    
    // Update SetScript Ana Sayfa
    console.log('Updating SetScript Ana Sayfa...');
    const { error: error1 } = await supabase
      .from('services')
      .update({ url: 'https://setscript.com' })
    ...
    const { error: error2 } = await supabase
      .from('services')
      .update({ url: 'https://api.setscript.com' })
    ...
    const { error: error3 } = await supabase
      .from('services')
      .update({ url: 'https://docs.setscript.com' })
    ...
    const { error: error4 } = await supabase
      .from('services')
      .update({ url: 'https://ai.setscript.com' })
